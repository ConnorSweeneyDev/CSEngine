...
namespace cse
{
  ...
  void game::event()
  {
    ...
    {
      hook.call<void(const SDL_Event &)>(hooks::PRE_EVENT, state.active.window->state.event);
      state.active.window->event();
      state.active.scene.pointer->event(state.active.window->state.event);
      hook.call<void(const SDL_Event &)>(hooks::POST_EVENT, state.active.window->state.event);
    }
  }
  ...
  void game::input()
  {
    ...
    hook.call<void(const bool *)>(hooks::PRE_INPUT, state.active.window->state.input);
    state.active.window->input();
    state.active.scene.pointer->input(state.active.window->state.input);
    hook.call<void(const bool *)>(hooks::POST_INPUT, state.active.window->state.input);
  }

  void game::simulate()
  {
    if (state.active.phase != help::phase::CREATED) throw exception("Game must be created before simulation");
    hook.call<void(const float)>(hooks::PRE_SIMULATE, static_cast<float>(state.actual_poll_rate));
    ...
    hook.call<void(const float)>(hooks::POST_SIMULATE, static_cast<float>(state.actual_poll_rate));
  }

  void game::render()
  {
    if (state.active.phase != help::phase::CREATED) throw exception("Game must be created before rendering");
    hook.call<void(const double)>(hooks::PRE_RENDER, state.alpha);
    ...
    hook.call<void(const double)>(hooks::POST_RENDER, state.alpha);
  }
