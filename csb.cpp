...
int build()
{
  ...
  std::filesystem::create_directories("build/shader");
  csb::multi_task_run(
    [](const auto &file)
    {
      return std::format("{} -spirv -T {}_6_0 -E main {} -Fo build/shader/{}.spv",
                         csb::host_platform == WINDOWS ? "build\\dxc\\dxc.exe" : "./build/dxc/dxc",
                         file.extension() == ".vert" ? "vs" : "ps", file.string(), file.filename().string());
    },
    csb::files_from({"program/shader"}), {"build/shader/[filename].spv"});

  std::unordered_map<std::filesystem::path, std::tuple<int, int, std::vector<std::tuple<std::string, int, int>>>>
    texture_configs = {};
  ...
  while (std::getline(config_file, line))
  {
    ...
    std::vector<std::tuple<std::string, int, int>> group_configs = {};
  ...
  config_file.close();
  using tuple_type = std::tuple<std::vector<unsigned char>,
                                std::tuple<int, int, int, int, int, std::vector<std::tuple<std::string, int, int>>>>;
  csb::embed<tuple_type>(
    ...
    {nullptr,
     [](const auto &file, const auto &name, const auto &)
     {
       if (file.extension() == ".spv")
         return "  static constexpr std::array<const unsigned char, [1]> " + name + "_data = {\n    [0]};\n\n";
       else
         return "  static constexpr std::array<const unsigned char, [1]> " + name +
                "_image = {\n    [0]};\n"
                "[2]\n\n";
     },
     [](const auto &file)
     ...
     [&texture_configs](const auto &file) -> tuple_type
     {
       if (file.extension() == ".spv")
       {
         std::vector<unsigned char> shader_data;
       ...
       else
       {
         ...
         std::vector<unsigned char> image_vector(image_data, image_data + (width * height * channels));
     ...
     [](const auto &name, const auto &data)
     {
       ...
       auto &[width, height, channels, frame_width, frame_height, frame_groups] = texture_config;
       ...
       for (const auto &frame_group : frame_groups)
       {
         ...
         groups_result += "  static constexpr std::array<const compiled_texture::frame_data::group::frame, " +
                          std::to_string(end_frame - start_frame + 1) + "> " + name + "_" + group_name +
                          "_frames = {\n    {";
    ...
    {[](const auto &files)
     {
       ...
       {
         if (file.extension() == ".spv")
           result += "  extern const compiled_shader " + name + ";\n";
         else
           result += "  extern const compiled_texture " + name + ";\n";
       }
       return result + "}";
     },
     [](const auto &files)
     {
       ...
       for (const auto &[file, name, data] : files)
       {
         if (file.extension() == ".spv")
         {
           ...
           result += "  const compiled_shader " + name +
                     " = {\n"
         ...
         {
           const auto &[width, height, channels, frame_width, frame_height, frame_groups] = std::get<1>(data);
           result += "  const compiled_texture " + name +
                     " = {\n"
                     "    std::span<const unsigned char>(" +
                     name +
                     "_image),\n"
           ...
           {
             const auto &[group_name, start_frame, end_frame] = frame_group;
             result += "     {\"" + group_name + "\", {" + std::to_string(start_frame) + ", " +
                       std::to_string(end_frame) + ", std::span<const compiled_texture::frame_data::group::frame>(" +
                       name + "_" + group_name + "_frames)}}";
